...
public:
  ...
  std::vector<ResonatorParams> getScaledDecay(float ratio) {

    std::vector<ResonatorParams> scaledModel;
    scaledModel.reserve(metadata.resonators);

    for (int i = 0; i < metadata.resonators; i++) { 
        ResonatorParams tmp_p = {model[i].freq, model[i].gain, ratio*model[i].decay};
        scaledModel.push_back(tmp_p);
    }
...
private:
    ...
    int noteNameToMidi (std::string noteName) {
        ...
        if (findNote != midiNoteNames.end()){
            ...
            if (opt.v) printf("[Model] Error: note not found \'%s\'\n", noteName.c_str());
            return -1;
        }
    }
